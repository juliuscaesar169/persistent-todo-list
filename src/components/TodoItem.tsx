...
const TodoItem = ({ todo, toggleComplete, onUpdate, onDelete }: Props) => {
    ...
    <div className="todo-item">
        {editing ? (
            <>
                <div>
                    <input className='todo-update-input' type="text" value={editedValue} onChange={(e) => setEditedValue(e.target.value)}/>
                </div>
                <div>
                    {/* Cancel btn */}
                    ...
                    {/* Save btn */}
                    <button className='std-btn' type="submit" onClick={() => handleSave()}>
                        Save
                    </button>
                </div>
            </>
